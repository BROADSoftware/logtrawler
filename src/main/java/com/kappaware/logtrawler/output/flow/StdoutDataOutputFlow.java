...
public class StdoutDataOutputFlow implements OutputFlow {
	Log log = LogFactory.getLog(StdoutDataOutputFlow.class);
	
	public StdoutDataOutputFlow() {
		log.info(String.format("Data events will be displayed on stdout'"));
	}
	
	@Override
	public void output(OutputItem item) {
		if (item.getType() == Type.DATA_LINE) {
			System.out.println(String.format("%s: %s", item.getFile(), item.getData()));
		}
	}
